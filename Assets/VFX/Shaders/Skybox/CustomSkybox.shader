Shader "Skybox/Custom Panoramic" {
Properties {
    ...
    _SecondTint ("Tint Color 2", Color) = (.5, .5, .5, .5)
    [Gamma] _SecondExposure ("Exposure 2", Range(0, 8)) = 1.0
    [NoScaleOffset] _SecondTex ("Spherical  (HDR) 2", 2D) = "grey" {}
...
SubShader {
    ...
    Pass {
        ...
        fixed4 frag (v2f i) : SV_Target
        {
            ...
            half4 tex2 = tex2D (_SecondTex, tc);
            half3 c = DecodeHDR (tex, _MainTex_HDR);
            half3 c2 = DecodeHDR (tex2, _SecondTex_HDR);
            c = c * _Tint.rgb * unity_ColorSpaceDouble.rgb;
            c2 = c2 * _SecondTint.rgb * unity_ColorSpaceDouble.rgb;
            c *= _Exposure;
            c2 *= _SecondExposure;
            c = lerp(c, c2, _TextureBlend);
            return half4(c, 1);
        }
        ENDCG
    }
